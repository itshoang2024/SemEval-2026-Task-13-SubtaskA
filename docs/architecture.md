...
- `src/features.py` owns feature extraction. `CodeStyleExtractor` produces deterministic 05_v9 style features; `LLMPerplexityEngine` produces compression-backed 8-column PPL features and overwrites completed rows with optional Qwen-based NLL features.
- `src/tuning.py` owns score rank-normalization, top-k ratio application, and per-language shrinkage.
...
- Any change to `test.parquet` language handling must preserve the 05_v9 `Unknown` language-aware fallback unless competition data is confirmed to always include `language`.
- Any change to LLM model loading should document whether compression-backed PPL fallback remains acceptable.
