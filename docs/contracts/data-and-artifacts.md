...
- `language`: optional. If absent, all rows are treated as `Unknown`; language-aware prediction falls back to the tuned global ratio for that group.
...
- `ppl_train_v9p.npy`, `ppl_test_v9p.npy`, `ppl_sample_v9p.npy`: 8-column compression-backed PPL matrices with completed LLM rows overwritten.
- `sty_train_v9p.npy`, `sty_test_v9p.npy`, `sty_sample_v9p.npy`: 05_v9 style feature matrices after PPL and language one-hot columns are appended.
- `oof_v9p.npy`: out-of-fold base-model predictions for the combined train+validation set.
- `te_sum_v9p.npy`: accumulated test base-model predictions across folds.
- `sa_sum_v9p.npy`: accumulated sample base-model predictions across folds.
- `meta_te_v9p.npy`, `meta_sa_v9p.npy`: meta-learner scores.
...
- the current code version's interpretation of compression-backed PPL fallback features.
...
- Missing CUDA, Transformers, or a loadable LLM does not fail the pipeline; it keeps compression-backed 8-column PPL features.
