...
- GPU runtime recommended. The LLM perplexity stage requires CUDA for Qwen rows; without it, the pipeline uses compression-backed PPL features.
...
- Logs for all major phases: data loading, compression-backed PPL/LLM overwrite, style extraction, 5-fold stacking, meta-learner, ratio tuning, and submission writing.
...
- No CUDA: the LLM stage logs a warning and keeps compression-backed PPL features. The rest of the pipeline continues.
- Transformers/model unavailable: the LLM stage logs a warning or model-load failures and keeps compression-backed PPL features.
- Missing `test_sample.parquet`: ratio tuning falls back to `PipelineConfig.fallback_global_ratio`; no sample F1 is logged.
- Missing `language` in `test.parquet`: expected for current code. Test rows become `Unknown`, and language-aware prediction uses the tuned global ratio for that group.
